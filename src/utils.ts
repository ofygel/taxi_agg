...
export const isAdmin = (ctx: MyContext) => ADMIN_IDS.includes(ctx.from?.id || 0);
export const fmtPhone = (p?: string | null) => (!p ? '' : (p.startsWith('+') ? p : '+' + p.replace(/[^\d]/g, '')));
export const toRad = (d: number) => d * Math.PI / 180;
export const b64 = (s: string) => Buffer.from(s, 'utf8').toString('base64');
export const ub64 = (s: string) => Buffer.from(s, 'base64').toString('utf8');
...
export async function sendEphemeral(ctx: MyContext, text: string, extra: any = {}, ttl = TTL_INFO_MS) {
...
export async function replaceWith(ctx: MyContext, text: string, extra: any = {}, ttl = TTL_INFO_MS) {
  try {
